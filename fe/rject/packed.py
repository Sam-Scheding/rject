...
# basestring doesn't exist in Python three, so we have to define it
try:
  basestring
except NameError:
  basestring = str
...
class Elem(object):
    """Represents an HTML element. Packed translates the Elem('a') into Elem('a') with an optional
    ...
    for root, dirs, files in os.walk(src):
        for filename in files:
            if filename.endswith('.py'):
                ...
                full_py_path = os.path.join(dest, filename)
